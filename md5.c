...
unsigned char *
md5(const char *message, size_t n, unsigned char *digest) {
    char *m = malloc(n + 64);
    size_t b = n * 8;
    ...
    WORD_LIST *list;
{
    ...
    md5(list->word->word, strlen(list->word->word), digest);
    for (int i = 0; i < 16; i++)
        sprintf(hexdigest+(i*2), "%02x", digest[i]);
    hexdigest[32] = '\0';

    shell_MD5SUM = bind_variable("MD5SUM", hexdigest, 0);
